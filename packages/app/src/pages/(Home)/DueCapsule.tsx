export default function DueCapsule({ skeleton, count, category, important, onClick }) {
    ...
    return (
        <button
            type="button"
            onClick={onClick}
            className="relative flex h-full items-center overflow-hidden rounded-2xl surface-card border p-3 shadow-sm ring-1 ring-accent-blue/15 text-left transition hover:border-accent-blue/40 hover:ring-accent-blue/30"
        >
            <div className="absolute inset-0 inset-shadow-sm" />
            <div className="relative flex w-full flex-row items-center gap-2 text-primary">
